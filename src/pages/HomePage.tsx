...
export default function HomePage() {
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      setSubmoduleImages((prev) =>
        prev.filter((img) => img.path !== confirmImage.path),
      );
    } catch (e: unknown) {
    ...
    const restoreAndLoad = async () => {
      ...
      try {
        ...
        if (segments.length >= 3 && newCrumbs.length === 3) {
          ...
          const sub = newCrumbs[2].items.find(
      ...
      <section className="bg-white rounded-xl shadow-sm border border-border p-6 mb-24">
        ...
        {isAtSubmodule && (
          ...
          >
            ...
            {uploadLoading
              ? "Загрузка..."
              : "Перетащите изображение или нажмите — сохранится в images/"}
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
            ...
            {imagesLoading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {submoduleImages.map((img) => (
                  <div
                    key={img.path}
                    className="group relative rounded-lg border border-border overflow-hidden bg-gray-50"
                  >
                    <img
                      src={img.content_url}
                      alt={img.name}
